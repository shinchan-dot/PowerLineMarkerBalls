...
namespace SaccFlightAndVehicles
{
    ...
    public class DFUNC_TrailSmoke : UdonSharpBehaviour
    {
        ...
        private bool Engine;
        public float SpeedConversion = 1.9438445f; // meters to knots
        private float[] HueArray = new float[] { 240f, 234f, 231f, 228f, 225f, 222f, 219f, 216f, 213f, 210f, 207f, 204f, 201f, 198f, 195f, 192f, 189f, 186f, 183f, 180f, 171f, 159f, 147f, 132f, 108f, 99f, 93f, 87f, 81f, 78f, 75f, 72f, 69f, 66f, 63f, 60f, 57f, 54f, 51f, 48f, 45f, 42f, 39f, 36f, 33f, 30f, 27f, 24f, 21f, 18f, 15f, 12f, 9f, 6f, 0f, -12f, -24f, -36f, -48f, -60f, };
        [UdonSynced, System.NonSerializedAttribute] public float SmokeHue = 1;
        [System.NonSerializedAttribute] private float SmokeHueLast = 1;
        ...
        */
        ...
        private void LateUpdate()
        {
            ...
            if (Engine)
            {
                if (AllowChangeColor)
                {
                    if (localSmoking)
                    {
                        ...
                        */
                        float airSpeed = (float)SAVControl.GetProgramVariable("AirSpeed") * SpeedConversion;
                        SmokeHue = SpeedToColor(airSpeed);

                        if (SmokeHue != SmokeHueLast && (Time.time - LastSerialization > .5f))
                        {
                            RequestSerialization();
                            LastSerialization = Time.time;
                        }
                        SmokeHueLast = SmokeHue;
                    }
                }
            }
            if (localSmoking && AllowChangeColor)
            {
                ...
                SmokeColor_Color = Color.HSVToRGB(SmokeHue / 360, 1, 1);
                foreach (ParticleSystem smoke in DisplaySmoke)
                {
                    var main = smoke.main;
                    main.startColor = SmokeColor_Color;
        ...
        private float SpeedToColor(float speed)
        {
            int i = Mathf.Clamp(Mathf.FloorToInt(speed / 100 * (HueArray.Length - 1)), 0, HueArray.Length - 1);
            return HueArray[i];
        }
    }
}
