...
namespace SaccFlightAndVehicles
{
    ...
    public class SaccAirVehicle : UdonSharpBehaviour
    {
        ...
        //** SH-1T **
        [Header("SH-1T:")]
        [Tooltip("Pitching/Rolling on the ground?")]
        public bool ThrustVectoringOnGround = true;
        [Tooltip("Joystick uses controller position instead controller rotation for input?")]
        public bool LongJoystick = false;
        [Tooltip("Joystick sensitivity. Distance at which joystick will reach maximum deflection in VR when the joystick is long")]
        public float MaxLongJoystickDistance = 0.15f;
        //** SH-1T **
            ...
            if (IsOwner)//works in editor or ingame
            {
                    ...
                    if (!_DisablePhysicsAndInputs)
                    {
                        ...
                        if (Grabbing || GrabToggle)
                        {
                            ...
                            * Quaternion.Inverse(JoystickZeroPoint)
                            ...
                            //** SH-1T **
                            // Override pitch and roll input for long joystick
                            if (LongJoystick)
                            {
                                Vector3 joystickPosition = ControlsRoot.InverseTransformPoint(localPlayer.GetTrackingData(SwitchHandsJoyThrottle ? VRCPlayerApi.TrackingDataType.LeftHand : VRCPlayerApi.TrackingDataType.RightHand).position);
                                if (!JoystickGripLastFrame)
                                ...
                                VRJoystickPos.x = Mathf.Clamp((joystickPosition.z - JoystickZeroPosition.z) / MaxLongJoystickDistance, -1.0f, 1.0f);
                                VRJoystickPos.z = Mathf.Clamp((joystickPosition.x - JoystickZeroPosition.x) / MaxLongJoystickDistance, -1.0f, 1.0f);
                            }
                            //** SH-1T **
                ...
                if (!_DisablePhysicsAndInputs)
                {
                    ...
                    if (!Asleep)//optimization
                    {
                        ...
                        if (ThrustVectoringOnGround)
                        {
                            pitch *= Mathf.Max(PitchThrustVecMulti, minlifttemp);
                            yaw *= Mathf.Max(YawThrustVecMulti, minlifttemp);
                            roll *= Mathf.Max(RollThrustVecMulti, minlifttemp);
                        }
                        else
                        {
                            pitch *= Mathf.Max(PitchThrustVecMulti * ThrustVecGrounded, minlifttemp);
